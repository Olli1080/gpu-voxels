...
namespace gpu_voxels {
	...
	VisVoxelMap::VisVoxelMap(voxelmap::AbstractVoxelMap* voxelmap, std::string map_name) :
		...
		m_shm_mapDim(nullptr), /**/
		m_shm_VoxelSize(nullptr), /**/
		m_shm_voxelmap_type(nullptr), /**/
		m_shm_voxelmap_changed(nullptr)
	{
	}

	VisVoxelMap::~VisVoxelMap() = default;
	...
	bool VisVoxelMap::visualize(const bool force_repaint)
	{
		...
		if (m_shm_memHandle == nullptr)
		{
			...
			const std::pair<uint32_t*, std::size_t> r = m_segment.find<uint32_t>(shm_variable_name_number_of_voxelmaps.c_str());
			if (r.second == 0)
			...
			m_shm_memHandle = m_segment.find_or_construct<cudaIpcMemHandle_t>(std::string(shm_variable_name_voxelmap_handler_dev_pointer + id).c_str())(cudaIpcMemHandle_t());
			m_shm_mapDim = m_segment.find_or_construct<Vector3ui>(std::string(shm_variable_name_voxelmap_dimension + id).c_str())(Vector3ui::Zero());
			m_shm_VoxelSize = m_segment.find_or_construct<float>(std::string(shm_variable_name_voxel_side_length + id).c_str())(0.0f);
			m_shm_mapName = m_segment.find_or_construct_it<char>(std::string(shm_variable_name_voxelmap_name + id).c_str())[m_map_name.size()](m_map_name.data());
			m_shm_voxelmap_type = m_segment.find_or_construct<MapType>(std::string(shm_variable_name_voxelmap_type + id).c_str())(m_voxelmap->getMapType());
			m_shm_voxelmap_changed = m_segment.find_or_construct<bool>(std::string(shm_variable_name_voxelmap_data_changed + id).c_str())(true);
		}
