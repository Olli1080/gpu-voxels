	...
	namespace voxellist
	{
		...
		bool TemplateVoxelList<CountingVoxel, MapVoxelID>::merge(const GpuVoxelsMapSharedPtr other, const Vector3i& voxel_offset, const BitVoxelMeaning* new_meaning)
		{
			...
			case MT_BITVECTOR_VOXELLIST:
			{
				...
				parallel::copy(
					parallel::make_zip_iterator(parallel::make_tuple(m->m_dev_coord_list.begin(), m->m_dev_id_list.begin())),
					parallel::make_zip_iterator(parallel::make_tuple(m->m_dev_coord_list.end(), m->m_dev_id_list.end())),
					parallel::make_zip_iterator(parallel::make_tuple(m_dev_coord_list.begin() + offset_new_entries, m_dev_id_list.begin() + offset_new_entries)));
				...
				if (voxel_offset != Vector3i::Zero())
				{
					parallel::transform(
						parallel::make_zip_iterator(parallel::make_tuple(m_dev_coord_list.begin() + offset_new_entries, m_dev_id_list.begin() + offset_new_entries)),
						parallel::make_zip_iterator(parallel::make_tuple(m_dev_coord_list.end(), m_dev_id_list.end())),
						parallel::make_zip_iterator(parallel::make_tuple(m_dev_coord_list.begin() + offset_new_entries, m_dev_id_list.begin() + offset_new_entries)),
			...
			case MT_COUNTING_VOXELLIST:
			{
				...
				// We append the given list to our own list of points.
				parallel::copy(
					parallel::make_zip_iterator(parallel::make_tuple(m->m_dev_list.begin(), m->m_dev_coord_list.begin(), m->m_dev_id_list.begin())),
					parallel::make_zip_iterator(parallel::make_tuple(m->m_dev_list.end(), m->m_dev_coord_list.end(), m->m_dev_id_list.end())),
					parallel::make_zip_iterator(parallel::make_tuple(m_dev_list.begin() + offset_new_entries, m_dev_coord_list.begin() + offset_new_entries, m_dev_id_list.begin() + offset_new_entries)));
				GVL_HANDLE_ERROR(GVL_SYNCHRONIZE());
		...
		bool TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>::merge(const GpuVoxelsMapSharedPtr other, const Vector3i& voxel_offset, const BitVoxelMeaning* new_meaning)
		{
			...
			case MT_BITVECTOR_VOXELLIST:
			{
				...
				if (new_meaning)
				{
					...
					parallel::fill(m_dev_list.begin() + offset_new_entries, m_dev_list.end(), fillVoxel);
					GVL_HANDLE_ERROR(GVL_SYNCHRONIZE());
				}
			...
			case MT_BITVECTOR_VOXELMAP:
			{
				...
				const size_t num_new_entries = parallel::count_if(
					parallel::device_ptr<BitVectorVoxel>(m->getDeviceDataPtr()),
					parallel::device_ptr<BitVectorVoxel>(m->getDeviceDataPtr() + m->getVoxelMapSize()),
				...
				parallel::copy_if(
					parallel::counting_iterator<MapVoxelID>(0),                     // src.begin
					parallel::counting_iterator<MapVoxelID>(m->getVoxelMapSize()),  // src.end
					parallel::device_ptr<BitVectorVoxel>(m->getDeviceDataPtr()),    // stencil.begin (predicate is used here)
			...
			default:
			{
				LOGGING_ERROR_C(VoxellistLog, TemplateVoxelList, GPU_VO_MAP_OPERATION_NOT_YET_SUPPORTED << endl);
				return false;
			}
			}
		}
		...
		template size_t TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>::collideVoxellists(const TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>* other, const Vector3i& offset, parallel::device_vector<bool>& collision_stencil) const;
		template size_t TemplateVoxelList<CountingVoxel, MapVoxelID>::collideVoxellists(const TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>* other, const Vector3i& offset, parallel::device_vector<bool>& collision_stencil) const;
		template size_t TemplateVoxelList<ProbabilisticVoxel, MapVoxelID>::collideVoxellists(const TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>* other, const Vector3i& offset, parallel::device_vector<bool>& collision_stencil) const;
		...
		//template size_t TemplateVoxelList<DistanceVoxel, MapVoxelID>::collideVoxellists(const TemplateVoxelList<BitVoxel<BIT_VECTOR_LENGTH>, MapVoxelID>* other, const Vector3i& offset, parallel::device_vector<bool>& collision_stencil) const;
	}
}