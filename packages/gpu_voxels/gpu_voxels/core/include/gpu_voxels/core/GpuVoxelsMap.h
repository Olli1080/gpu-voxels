...
namespace gpu_voxels {
	...
	concept explicitly_convertible_to = requires(From t) { static_cast<To>(t); };

	class GpuVoxelsMap;
	typedef std::shared_ptr<GpuVoxelsMap> GpuVoxelsMapSharedPtr;

	class GpuVoxelsMap
	{
	public:
		//! Constructor
		GpuVoxelsMap() = default;

		//! Destructor
		virtual ~GpuVoxelsMap() = default;

		//to ensure these methods can't be called
		GpuVoxelsMap(const GpuVoxelsMap&) = delete;
		const GpuVoxelsMap& operator=(const GpuVoxelsMap&) = delete;

		/*!
		 *
		 */
		template<class T>
		const T* as() const requires explicitly_convertible_to<GpuVoxelsMap*, T*>
		{
		 ...
		 */
		template<class T>
		T* as() requires explicitly_convertible_to<GpuVoxelsMap*, T*>
		{
		...
		virtual void insertPointCloud(const PointCloud& pointcloud, BitVoxelMeaning voxel_meaning) = 0;


		virtual void insertCoordinateList(const std::vector<Vector3ui>& coordinates, BitVoxelMeaning voxel_meaning) = 0;

		virtual void insertCoordinateList(const parallel::device_vector<Vector3ui>& d_coordinates, BitVoxelMeaning voxel_meaning) = 0;
		...
		virtual bool insertMetaPointCloudWithSelfCollisionCheck(const MetaPointCloud* robot_links,
			...
			BitVector<BIT_VECTOR_LENGTH>* colliding_meanings = nullptr) = 0;
		 ...
		 */
		 //virtual size_t collideWith(const GpuVoxelsMapSharedPtr other, float coll_threshold = 1.0, const Vector3i &offset = Vector3i::Zero()) = 0;
		  ...
		  * \return The severity of the collision, namely the number of voxels that lie in collision
		  */
		  //virtual size_t collideWithTypes(const GpuVoxelsMapSharedPtr other, BitVectorVoxel&  meanings_in_collision, float coll_threshold = 1.0, const Vector3i &offset = Vector3i::Zero()) = 0;
		   ...
		   * \return The severity of the collision, namely the number of voxels that lie in collision
		   */
		   //virtual size_t collideWithBitcheck(const GpuVoxelsMapSharedPtr other, const u_int8_t margin = 0, const Vector3i &offset = Vector3i::Zero()) = 0;
		 ...
		 * \param new_meaning If not nullptr, the meaning of the copied Voxels is set to this meaning.
		 * \return true if succeeded, false otherwise
		 */
		virtual bool merge(GpuVoxelsMapSharedPtr other, const Vector3f& metric_offset, const BitVoxelMeaning* new_meaning = nullptr) = 0;
		 ...
		 * \return true if succeeded, false otherwise
		 */
		virtual bool merge(GpuVoxelsMapSharedPtr other, const Vector3i& voxel_offset = Vector3i::Zero(), const BitVoxelMeaning* new_meaning = nullptr) = 0;
		 ...
		 * \param path filename
		 */
		virtual bool writeToDisk(std::string path) = 0;
		 ...
		 * \return true if succeeded, false otherwise
		 */
		virtual bool readFromDisk(std::string path) = 0;
