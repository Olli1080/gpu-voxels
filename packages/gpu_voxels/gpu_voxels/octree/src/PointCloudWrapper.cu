...
namespace gpu_voxels
{
    namespace NTree
    {
		...
		{
			GVL_LAUNCH_KERNEL(kernel_transformKinectPoints, NUM_BLOCKS, NUM_THREADS_PER_BLOCK, d_point_cloud, num_points, d_tmp_voxel, d_sensor, voxel_dimension);
		}
		    
		void kernel_countVoxel_start(Voxel* d_tmp_voxel, voxel_count num_points, OctreeVoxelID* count_voxel)
		{
			GVL_LAUNCH_KERNEL(kernel_countVoxel, NUM_BLOCKS, NUM_THREADS_PER_BLOCK, d_tmp_voxel, num_points, count_voxel);
		}
		...
		{
			GVL_LAUNCH_KERNEL(kernel_combineEqualVoxel, NUM_BLOCKS, NUM_THREADS_PER_BLOCK, d_tmp_voxel, num_voxel, count_voxel, voxel, d_sensor);
		}
		...
		{
			GVL_LAUNCH_KERNEL(kernel_voxelize_finalStep, num_blocks, num_threads,d_tmp_voxel_id,
			  num_points,
			  num_voxel,
			  d_voxel,
			  d_sensor);
