...
namespace gpu_voxels
{
	namespace NTree
	{
				...
				int* shared_mem)
		{
			...
			const uint32_t log_free = static_cast<uint32_t>(log2f(ns_FREE));
			const uint32_t log_unknown = static_cast<uint32_t>(log2f(ns_UNKNOWN));
			const uint32_t log_occupied = static_cast<uint32_t>(log2f(ns_OCCUPIED));
			const uint32_t log_static = static_cast<uint32_t>(log2f(ns_STATIC_MAP));
			const uint32_t log_dynamic = static_cast<uint32_t>(log2f(ns_DYNAMIC_MAP));
			...
			if (do_final_computation)
			{
				...
				const NodeStatus my_status_or = (my_free_votes > 0) << log_free | (my_unknown_votes > 0) << log_unknown
		...
		{
			const uint32_t coordinate_step = static_cast<uint32_t>(const_cube_side_length[level]);
			...
			new_coordinate_max = coordinate + gpu_voxels::Vector3ui::Constant(coordinate_step) + shift;
