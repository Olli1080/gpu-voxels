...
namespace gpu_voxels
{
	namespace NTree
	{
		...
		void kernel_countNodes(OctreeVoxelID* const voxel, const OctreeVoxelID numVoxel, const uint32_t level, OctreeVoxelID* const sum)
		{
			...
			const OctreeVoxelID to = min(from + chunk_size, numVoxel);
		...
		{
			const uint32_t chunk_size = ceil(static_cast<double>(numVoxel) / (gridDim.x * blockDim.x));
			const OctreeVoxelID id = blockIdx.x * blockDim.x + threadIdx.x;
			const OctreeVoxelID from = chunk_size * id;
			const OctreeVoxelID to = min(from + chunk_size, numVoxel);
		...
		void kernel_print(InnerNode* root, InnerNode* stack1, InnerNode* stack2)
		{
			...
			for (int32_t level = level_count - 2; level >= 0; --level)
			{
				...
				while (stack1Top >= 0)
				{
					...
					{
						assert((level == 0 && node.hasStatus(ns_LAST_LEVEL)) || (level != 0 && !node.hasStatus(ns_LAST_LEVEL)));
						if (node.hasStatus(ns_LAST_LEVEL))
						{
							LeafNode* child = (LeafNode*)node.getChildPtr();
							printf("Address: %016zX ", reinterpret_cast<uintptr_t>(child));
			...
			for (int32_t level = level_count - 1; level >= 0; --level)
			{
				...
				while (stack1Top >= 0)
				{
					...
					if (current.m_c)
					{
						...
						const uint8_t s = tmp_leaf->getStatus();
						getStatusString(status, s);
						printf("[%zu][%s][@ ] -- ", current.m_b, status);  // tmp_leaf);
					}
					else
					{
						char status[8];
						...
						printf("[%zu - %zu][%s][@ ] -- ", 
							current.m_b,
							current.m_b + static_cast<size_t>(powf(branching_factor, level) - 1),
							static_cast<char*>(status));  // node);
      ...
      { \
				...
				voxel_count* num_collisions, VoxelFlags* result_voxelFlags)
		{
				...
				i += STEPS * (gridDim.x * blockDim.x))
			{
				...
				InnerNode* node = root;
				const int common_level = getCommonLevel<branching_factor>(nodeID0, nodeID1);
			...
			const Vector3i voxelmap_offset = Vector3i::Zero())
		{
			...
			for (voxel_count i = blockIdx.x * blockDim.x + threadIdx.x; i < voxelmap_size; i += gridDim.x * blockDim.x)
			{
				...
				{
					const OctreeVoxelID nodeID = morton_code60((voxelmap::mapToVoxels(voxels, dimensions, my_voxel).template cast<int64_t>() + voxelmap_offset.template cast<int64_t>()).template cast<uint32_t>());
					InnerNode* node = root;
					int level = level_count - 2;
					const int tmp_level = level;
					InnerNode* tmp_node = node;
			...
			const Vector3i voxelmap_offset = Vector3i::Zero())
		{
			...
			for (voxel_count i = blockIdx.x * blockDim.x + threadIdx.x; i < voxellist_size; i += gridDim.x * blockDim.x)
			{
				...
				if (isVoxelOccupied(my_voxel))
				{
					const Vector3ui tmp = (voxel_coords[i].cast<int64_t>() + voxelmap_offset.template cast<int64_t>()).template cast<uint32_t>();
					const OctreeVoxelID nodeID = morton_code60(tmp);
					InnerNode* node = root;
					int level = level_count - 2;
					const int tmp_level = level;
					InnerNode* tmp_node = node;
...
#pragma unroll
				...
				OctreeVoxelID* numConflicts, const uint32_t splitLevel)
		{
			...
			a_LeafNode* a_leaf = nullptr;
			b_LeafNode* b_leaf = nullptr;
		...
		struct BitMapProperties
		{
				...
				__forceinline__ T* computePtr(uint32_t x, uint32_t y, uint32_t z, uint32_t* bit_index)
			{
				if (min_x <= x && x <= max_x && min_y <= y && y <= max_y && min_z <= z && z <= max_z)
				{
					...
					const uint32_t x_bit_index = x & (1 << (log_branching_factor_third_root - 1));
					const uint32_t y_bit_index = y & (1 << (log_branching_factor_third_root - 1));
					const uint32_t z_bit_index = z & (1 << (log_branching_factor_third_root - 1));
...
#undef SWAP_FROM_TO
		...
		{
			const auto branching_factor_third_root = static_cast<uint32_t>(powf(branching_factor, 1.0 / 3));
		...
		__global__
			static void kernel_packByteMap_MemEfficient_Coa2(voxel_count* num_this_level, voxel_count* num_next_level,
				...
				voxel_count* index_next_level = nullptr,
				OctreeVoxelID* voxel_id_this_level = nullptr,
				OctreeVoxelID* voxel_id_next_level = nullptr)
		{
			...
			constexpr uint32_t NUM_THREADS = WARP_SIZE * NUM_WARPS;
			assert(NUM_THREADS == blockDim.x);
			...
			const uint32_t free_mask[4] = { 1, 1 << 8, 1 << 16, 1 << 24 };
...
#pragma unroll
				for (uint32_t i = 0; i < 2; ++i)
				{
					...
					const bool occupied = (((is_occupied >> (i * 16)) & is_free_mask) == is_free_mask);
					const uint32_t all_votes = BALLOT(occupied);
...
#pragma unroll
							...
							{
								if (((free_mask[i * 2 + k] & is_occupied) != 0) & (voxel_id_this_level != nullptr))
								{
...
#pragma unroll
		...
		__global__
			static void kernel_packMortonL0Map(
				...
				OctreeVoxelID* voxel_id_this_level = nullptr,
				OctreeVoxelID* voxel_id_next_level = nullptr,
				typename InnerNode::NodeData::BasicData* basic_data_this_level = nullptr,
				typename InnerNode::NodeData::BasicData* basic_data_next_level = nullptr,
				...
				MapProperties<typename InnerNode::RayCastType, branching_factor>())
		{
			...
			//    blockCopy(shared_data, map_ptr + i, work_size * sizeof(T), thread_id, num_threads);
				...
				if (!COUNT_MODE)
				{
					...
					const uint32_t this_level_votes = thread_prefix<num_warps>((uint32_t*)shared_this_level_prefix, thread_id,
					...
					else if (PACKING)
					{
						...
						const uint32_t next_level_votes = thread_prefix<num_warps>((uint32_t*)shared_next_level_prefix, thread_id,
				...
				OctreeVoxelID* free_space_wo_duplicates, voxel_count* voxel_count_wo_duplicates)
		{
			...
			{
				const voxel_count my_id = i + thread_id;
				const OctreeVoxelID my_element = free_space[my_id];
				const bool is_no_duplicate = (my_id < to) && ((my_id == 0) || (my_element != free_space[my_id - 1]))
				...
				else
				{
					...
					const uint32_t votes = BALLOT(is_no_duplicate);
					if (is_no_duplicate)
					{
				...
				OctreeVoxelID* free_space_this_level, OctreeVoxelID* free_space_next_level)
		{
			...
			for (voxel_count i = from; i < to; i += num_threads)
			{
				...
				{
					// ##### handle next level #####
					const bool next_level_vote = (num_childs_split_parent == 0) & (is_active)&has_new_parent & !is_this_level;
					const uint32_t next_level_votes = BALLOT(next_level_vote);
					if (next_level_vote)
					{
						const uint32_t next_level_index = __popc(next_level_votes << (32 - thread_id));
					...
					{
						const uint32_t this_level_index = __popc(shared_votes_this_level << (32 - thread_id));
				...
				if (shared_num_childs_split_parent != 0)
				{
					...
					else
					{
						...
						{
							// ##### handle this level #####
							if ((thread_id < shared_num_with_same_parent) & (free_space_this_level != nullptr))
							{
			...
			static void kernel_checkBlub(OctreeVoxelID* voxel_id1, voxel_count num_voxel, OctreeVoxelID* voxel_id2)
		{
			for (uint32_t i = 0; i < num_voxel; ++i)
			{
				...
				{
					printf("pos %u voxelList %zu vs. %zu\n", i, voxel_id1[i], voxel_id2[i]);
					break;
				}
			}
		}
			...
			static void kernel_checkTree(InnerNode* root, uint8_t* error)
		{
			...
			*error = 0;
			...
			const OctreeVoxelID last_voxel = (OctreeVoxelID)powf(branching_factor, level_count - 1);
			for (OctreeVoxelID i = 0; i < last_voxel && !*error;)
			{
				...
				{
					const bool result = isValidParentStatus(shared_stack[l]->getChildPtr(), branching_factor, l - 1,
						shared_stack[l]);
					if (!result)
					{
						printf("ERROR InnerNode on level %u for voxel with id %zu has wrong status flags\n", l, i);
						//          printf(
