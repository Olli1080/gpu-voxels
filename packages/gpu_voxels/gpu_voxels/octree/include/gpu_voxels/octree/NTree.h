...
namespace gpu_voxels {
	namespace NTree {
		...
		{
		protected:
			static constexpr uint32_t INITIAL_REBUILD_BUFFER_SIZE = 4000000;
			static constexpr uint32_t INITIAL_EXTRACT_BUFFER_SIZE = 100000;
		...
		public:
			...
			voxel_count intersect_sparse(const gpu_voxels::voxelmap::TemplateVoxelMap<VoxelType>& voxel_map,
				VoxelType* h_result_voxel = nullptr, const uint32_t min_level = 0,
				gpu_voxels::Vector3i offset = gpu_voxels::Vector3i(0, 0, 0), voxel_count* num_colls_with_unknown_cells = nullptr);
			 ...
			 */
			template<bool set_collision_flag, bool compute_voxelTypeFlags, bool compute_collsWithUnknown, typename VoxelType>
			voxel_count intersect_sparse(const gpu_voxels::voxellist::TemplateVoxelList<VoxelType, MapVoxelID>& voxel_map,
				BitVectorVoxel* h_result_voxel = nullptr, const uint32_t min_level = 0,
				gpu_voxels::Vector3i offset = gpu_voxels::Vector3i(0, 0, 0), voxel_count* num_colls_with_unknown_cells = nullptr);
			 ...
			 */
			template<bool set_collision_flag, bool compute_voxelTypeFlags, bool compute_collsWithUnknown, typename VoxelType>
			voxel_count intersect_morton(const gpu_voxels::voxellist::TemplateVoxelList<VoxelType, OctreeVoxelID>& voxel_list,
				BitVectorVoxel* h_result_voxel = nullptr, const uint32_t min_level = 0, voxel_count* num_colls_with_unknown_cells = nullptr);
			...
			voxel_count intersect_load_balance(const gpu_voxels::voxelmap::ProbVoxelMap& voxel_map,
				gpu_voxels::Vector3i offset = gpu_voxels::Vector3i(0, 0, 0),
				const uint32_t min_level = 0,
				BitVector<vft_size>* h_result_voxelTypeFlags = nullptr);
			...
			OctreeVoxelID intersect_load_balance(const NTree<branching_factor, level_count, o_InnerNode, o_LeafNode>* other,
				const uint32_t min_level, Collider collider = DefaultCollider(), bool mark_collisions = true,
				double* balance_overhead = nullptr, int* num_balance_tasks = nullptr);
			...
			struct Trafo_OctreeVoxelID
			{
				...
				__host__ __device__ OctreeVoxelID operator()(OctreeVoxelID x)
				{
					...
					return morton_code60(coordinates.x() * m_scale, coordinates.y() * m_scale, coordinates.z() * m_scale);
				}
			};
