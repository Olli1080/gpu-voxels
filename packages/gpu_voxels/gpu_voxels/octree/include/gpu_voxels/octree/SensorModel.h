		...
		public:
				...
				Probability estimateVoxelProbability(Voxel* first, Voxel* last) const
			{
				...
				for (Voxel* i = first + 1; i < last; ++i)
					...
					occ = MIN(MAX(static_cast<int32_t>(static_cast<int32_t>(occ) + static_cast<int32_t>(i->getOccupancy())), static_cast<int32_t>(MIN_PROBABILITY)), static_cast<int32_t>(MAX_PROBABILITY));
				return occ;
			}
			...
			{
				// watch out for overflow: cast to int32_t
				return MIN(MAX(static_cast<int32_t>(static_cast<int32_t>(initial_probability) + static_cast<int32_t>(update_probabilty) * count), static_cast<int32_t>(MIN_PROBABILITY)), static_cast<int32_t>(MAX_PROBABILITY));
			}
