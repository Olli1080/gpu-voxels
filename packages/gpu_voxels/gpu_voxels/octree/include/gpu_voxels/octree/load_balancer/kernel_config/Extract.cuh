						...
						Variables& variables, const Constants& constants, KernelParams& kernel_params)
				{
					...
					if (variables.is_active)
					{
						...
						for (uint32_t i = 0; i < branching_factor; ++i)
						{
							...
							const uint32_t votes = BALLOT(is_leaf_vote);
							if ((constants.thread_id % WARP_SIZE) == 0)
					...
					const uint32_t votes_last_level = BALLOT(is_last_level);
					const uint32_t votes_is_part = BALLOT(is_part);
					const uint32_t votes_is_active = BALLOT(variables.is_active);
					const uint32_t votes_is_min_level = BALLOT(is_min_level);
					const uint32_t votes_is_selected = BALLOT(is_selected);
