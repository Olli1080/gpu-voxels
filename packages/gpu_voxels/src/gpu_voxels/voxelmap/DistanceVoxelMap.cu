...
namespace gpu_voxels {
	namespace voxelmap {
		...
		bool DistanceVoxelMap::mergeOccupied(const std::shared_ptr<ProbVoxelMap> other, const Vector3ui& voxel_offset, float occupancy_threshold) {
		...
		void DistanceVoxelMap::parallelBanding3D(uint32_t m1, uint32_t m2, uint32_t m3, uint32_t arg_m1_blocksize, uint32_t arg_m2_blocksize, uint32_t arg_m3_blocksize, bool detailtimer)
		{
			...
			//optimize m1,m2,m3; m3 is especially detrimental? (increases divergence)
			// m2, m3 works on dim.y first, then dim.x after transpose
			m1 = (std::max)(static_cast<uint32_t>(1), (std::min)(m1, this->m_dim.z())); //band count in phase1
			m2 = (std::max)(static_cast<uint32_t>(1), (std::min)(m2, this->m_dim.y())); //band count in phase2
			m3 = (std::max)(static_cast<uint32_t>(1), (std::min)(m3, this->m_dim.y())); //band count in phase3 //TODO:: is bug? should this be n_dim.x() ?
			...
			dim3 m1_block_size((std::min)(arg_m1_blocksize, this->m_dim.x())); // optimize blocksize
			dim3 m1_grid_size(this->m_dim.x() / m1_block_size.x, this->m_dim.y(), m1); //m1 bands

			if (this->getDimensions().x() < m1_block_size.x) 
			{
				//TODO: check for phase1-3 that m1-3 and block sizes are always safe and kernels will not cause memory access violations
			...
			dim3 m2_block_size((std::min)(arg_m2_blocksize, (std::min)(this->m_dim.x(), this->m_dim.y()))); // optimize blocksize
			...
			{
				//TODO: check for phase1-3 that m1-3 and blocksizes are always safe and kernels will not cause memory access violations
			...
			//TODO: use template specialization to implement; run once with and without textures
			...
			cudaCreateTextureObject(&initialTexObj, &initialResDesc, &texDesc, nullptr);
			...
			dim3 m3_block_size((std::min)(arg_m3_blocksize, (std::min)(this->m_dim.x(), this->m_dim.y())), m3); // y bands; block_size threads in total
		...
		// phase 3 done: distance_map contains final result
			...
			// optimize: check bare transpose performance in-place vs non-inplace
		...
		{
			return static_cast<DistanceVoxel::pba_dist_t>(sqrt(this->getSquaredObstacleDistance(pos.x(), pos.y(), pos.z())));
		}

		DistanceVoxel::pba_dist_t DistanceVoxelMap::getObstacleDistance(uint x, uint y, uint z) const
		{
			return static_cast<DistanceVoxel::pba_dist_t>(sqrt(this->getSquaredObstacleDistance(x, y, z)));
		}
		 ...
		 * @return
		 */
		DistanceVoxel::accumulated_diff DistanceVoxelMap::differences3D(const std::shared_ptr<DistanceVoxelMap> other, int debug, bool logging_reinit) {
			...
			if (debug > 1) {
				...
				uintptr_t obstacle_idx = getVoxelIndexUnsigned(m_dim, obstacle);
				for (int i = -2; i <= 2; i++) {
					idx = static_cast<intptr_t>(obstacle_idx) + i;
				...
				for (int dz = -1; dz <= 1; dz++) {
					for (int dy = -1; dy <= 1; dy++) {
						for (int dx = -1; dx <= 1; dx++) {
							Vector3ui voxel((std::max)(obstacle.x() + dx, static_cast<uint32_t>(0)), (std::max)(obstacle.y() + dy, static_cast<uint32_t>(0)), (std::max)(obstacle.z() + dz, static_cast<uint32_t>(0)));
			...
			if (debug != 0) {
				...
				if ((result.count > 0) && (result.count < 250)) { //don't print differing obstacles at identical distance?
					for (size_t i = 0; i < host_map.size(); i++) {
						...
						int32_t dd = d.squaredObstacleDistance(mapToVoxelsSigned(idx, m_dim));
						const DistanceVoxel& od = host_other_map[i];
						int32_t odd = od.squaredObstacleDistance(mapToVoxelsSigned(idx, m_dim));
