...
namespace gpu_voxels {
	...
	public:
		...
		struct extract_byte_distance
		{
			...
			__host__ __device__
				...
				const int free_space = static_cast<int>(sqrt(static_cast<float>(dv.squaredObstacleDistance(pos))));
		...
		struct diff_op
		{
				...
				double operator()(const tuple_t ia, const tuple_t ib) const
			{
				...
				const double diff = sqrt(static_cast<double>(da)) - sqrt(static_cast<double>(db));
		 ...
		 */
		 ...
		 //template<> friend std::ostream& operator<<(std::ostream&, const DistanceVoxel&);
	...
	*/
	...
	template std::ostream& operator<<(std::ostream&, const DistanceVoxel&);
