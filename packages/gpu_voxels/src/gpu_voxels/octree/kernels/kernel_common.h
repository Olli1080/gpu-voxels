...
#pragma unroll
			for (uint32_t i = idx; i < (size >> 2); i += nThreads)
				static_cast<uint32_t*>(dst)[i] = static_cast<uint32_t*>(src)[i];

			if (idx < (size & 0x03))
				static_cast<char*>(dst)[size - idx - 1] = static_cast<char*>(src)[size - idx - 1];
		}
...
#pragma unroll
				...
				Environment::InnerNode* parent)
		{
			...
			for (uint32_t c = 0; c < node_count; ++c)
			{
				if (level == 0)
				{
					childs_status_flags |= static_cast<Environment::LeafNode*>(nodes)[c].getStatus();
				}
				else
					childs_status_flags |= static_cast<Environment::InnerNode*>(nodes)[c].getStatus();
			}
...
#undef USE_BALLOT
			...
			Probability updateOccupancy(const Probability old_value, const Probability new_value)
		{
			...
			return (std::min)((std::max)(static_cast<int32_t>(old_value) + static_cast<int32_t>(new_value), static_cast<int32_t>(MIN_PROBABILITY)), static_cast<int32_t>(MAX_PROBABILITY));
		}
		...
		const NodeStatus top_down_status_mask, const NodeStatus status_mask)
				  ...
				  Environment::InnerNodeProb* parent)
		  {
			  ...
			  for (uint32_t c = 0; c < node_count; ++c)
			  {
				  ...
				  NodeStatus st;
				  if (level == 0) {
					  tmp = static_cast<Environment::LeafNodeProb*>(nodes)[c].getOccupancy();
					  st = static_cast<Environment::LeafNodeProb*>(nodes)[c].getStatus();
					  all_non_part &= true;
				  }
				  else {
					  tmp = static_cast<Environment::InnerNodeProb*>(nodes)[c].getOccupancy();
					  all_non_part &= !static_cast<Environment::InnerNodeProb*>(nodes)[c].hasStatus(ns_PART);
					  st = static_cast<Environment::InnerNodeProb*>(nodes)[c].getStatus();
				  }
				  array[c] = st;

				  p_max = static_cast<Probability>((std::max)(static_cast<int>(tmp), static_cast<int>(p_max)));
				  p_min = static_cast<Probability>((std::min)(static_cast<int>(tmp), static_cast<int>(p_min)));
			  }
				  ...
				  const Environment::NodeProb::NodeData::BasicData reset_basic_data)
		  {
			  ...
			  if (childPtr == nullptr && !reset_basic_data.hasFlags(nf_UPDATE_SUBTREE) && node->hasStatus(ns_PART))
			  {
