	...
	namespace NTree
	{
				...
				Sensor* sensor, gpu_voxels::Vector3f voxel_dimension)
		{
			...
			for (OctreeVoxelID i = id; i < num_points; i += numThreads)
			{
				...
				Voxel* v = &voxel[i];
				v->coordinates.x() = static_cast<uint32_t>(point.x() / voxel_dimension.x());
				v->coordinates.y() = static_cast<uint32_t>(point.y() / voxel_dimension.y());
				v->coordinates.z() = static_cast<uint32_t>(point.z() / voxel_dimension.z());
				...
				OctreeVoxelID* voxel, Sensor* sensor, const uint32_t resolution)
		{
			for (voxel_count i = blockIdx.x * blockDim.x + threadIdx.x; i < num_points; i += gridDim.x * blockDim.x)
			{
				...
				c.x() = static_cast<uint32_t>(point.x() / resolution);
				c.y() = static_cast<uint32_t>(point.y() / resolution);
				c.z() = static_cast<uint32_t>(point.z() / resolution);
		...
		void kernel_toVoxels(const Vector3f* input_points, size_t num_points, Vector3ui* output_voxels, float voxel_side_length)
		{
			...
			for (voxel_count i = id; i < num_points; i += numThreads)
			{
				...
				Vector3ui voxel;
				voxel.x() = static_cast<uint32_t>(pointf.x() / voxel_side_length); // convert to meter and discrete in voxel
				voxel.y() = static_cast<uint32_t>(pointf.y() / voxel_side_length);  // convert to meter and discrete in voxel
				voxel.z() = static_cast<uint32_t>(pointf.z() / voxel_side_length);  // convert to meter and discrete in voxel
