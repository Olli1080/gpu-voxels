...
static std::mt19937 g_engine = std::mt19937(rd());
...
#undef SORT_ON_GPU
		...
		{
			Vector3f temp_max;
			Vector3f temp_min = temp_max = points[0];
		...
		void transformPoints(std::vector<Vector3f>& points, uint3comp* out_points, size_t num_points,
			Vector3f offset, float scaling)
		{
			for (size_t i = 0; i < num_points; i++)
			{
				uint3comp coordinates{};
				coordinates.x = static_cast<uint32_t>(floor((points[i].x() + offset.x()) * scaling));
				coordinates.y = static_cast<uint32_t>(floor((points[i].y() + offset.y()) * scaling));
				coordinates.z = static_cast<uint32_t>(floor((points[i].z() + offset.z()) * scaling));
		...
		Vector3ui getMapDimensions(std::vector<Vector3f>& point_cloud, Vector3f& offset, float scaling)
		{
			...
			const auto num_points = point_cloud.size();
			Vector3f min, max;
			...
			Vector3ui& map_dimensions, float scaling)
		{
			...
			auto transformed_points = std::vector<uint3comp>(num_points);
