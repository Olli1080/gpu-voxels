...
namespace icl_core {
	namespace config {
		...
		{
		public:
			ConfigList(const std::string& config_prefix,
			...
			bool get(std::string const& prefix, icl_core::logging::LogStream& log_stream) const override
			{
				...
				ConfigIterator cIter = ConfigManager::instance().find(
					std::regex_replace(prefix + m_config_prefix, std::regex("\\/"), "\\/") + "\\/([^\\/]+:?)(\\/(.+))?");
				while (cIter.next())
				{
					bool found = false;
			...
			std::string key() const override { return m_config_prefix; }
			std::string stringValue() const override { return m_string_value; }
...
auto CONFIG_LIST(auto prefix, auto members, auto result)
{
	return new icl_core::config::ConfigList<cls, decltype(result)>(prefix, members, result);
}
