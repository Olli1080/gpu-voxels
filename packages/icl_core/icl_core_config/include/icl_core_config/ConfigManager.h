...
namespace icl_core {
    namespace config {
        ...
        template<typename T>
        concept to_string_able = requires(T value)
        {
            std::to_string(value);
        };
        ...
        private:
            ...
            void readXml(const ::std::string& prefix, tinyxml2::XMLNode* node, FilePath fp, bool extend_prefix = true);
