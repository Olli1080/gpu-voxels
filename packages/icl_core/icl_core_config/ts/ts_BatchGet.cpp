...
std::vector<std::string> enum_value_description = { "ONE", "TWO", "THREE" };

char const* enum_value_char_description[] = {"ONE", "TWO", "THREE", nullptr};
...
auto MEMBER_ENUM(auto key, std::function<V& (T&)>&& f)
{
    MEMBER_ENUM_1("EnumEntry", ConfigListEntry, enum_value, enum_value_description)
    return new icl_core::config::MemberValue(key, f);
}



template<typename T, typename V>
auto MEMBER_VALUE(auto key, std::function<V&(T&)> &&f)
{
    return new icl_core::config::MemberValue(key, f);
}
*/
...
BOOST_AUTO_TEST_CASE(ConfigValue)
{
  ...
  bool read_success =
      icc::get(icl_core::config::ConfigValueList{ CONFIG_VALUE("/ConfigValue/StringEntry", string_value),
...
BOOST_AUTO_TEST_CASE(ConfigEnum)
{
  ...
  bool read_success =
      icc::get({ CONFIG_ENUM("/ConfigEnum/Entry1", value1, enum_value_char_description),
...
BOOST_AUTO_TEST_CASE(ConfigPrefix)
{
  ...
  bool read_success =
    icc::get("/ConfigPrefix",
        { CONFIG_VALUE("StringEntry", string_value),
...
BOOST_AUTO_TEST_CASE(ConfigList)
{
  ...
  bool read_successful =
      icc::get(icl_core::config::ConfigValueList{
	               CONFIG_LIST<ConfigListEntry>(
		               "/ConfigList/List",
                       std::initializer_list<icl_core::config::impl::MemberValueIface<ConfigListEntry>*>{
