...
namespace gpu_voxels {
	namespace visualization {
							...
							for (uint32_t j = y; j < dim_super_voxel.y() + y && j < dim_voxel_map.y() && !found; j++)
							{
								for (uint32_t k = z; k < dim_super_voxel.z() + z && k < dim_voxel_map.z() && !found; k++)
								{
									...
									if (voxel.getOccupancy() >= occupancy_threshold)
									{
										...
										uint8_t type = (std::max)((std::min)(static_cast<uint8_t>(eBVM_SWEPT_VOLUME_START + voxel.getOccupancy() - MIN_PROBABILITY), static_cast<uint8_t>(eBVM_SWEPT_VOLUME_END)), static_cast<uint8_t>(eBVM_SWEPT_VOLUME_START));
		...
		__global__ void fill_vbo_without_precounting(const BitVectorVoxel* voxelMap, Vector3ui dim_voxel_map,
			...
			uint32_t* write_index, const uint8_t* draw_voxel_type,
						...
						for (uint32_t i = x; i < dim_super_voxel.x() + x && i < dim_voxel_map.x() && !found; i++)
						{
							for (uint32_t j = y; j < dim_super_voxel.y() + y && j < dim_voxel_map.y() && !found; j++)
							{
								for (uint32_t k = z; k < dim_super_voxel.z() + z && k < dim_voxel_map.z() && !found; k++)
								{
									...
									{
										for (uint32_t t = 0; t < (std::min)((unsigned long long) BIT_VECTOR_LENGTH, (unsigned long long) MAX_DRAW_TYPES); ++t)
										{
											uint32_t index = 0xffff;
											uint8_t prefix;
							...
							for (uint32_t j = y; j < dim_super_voxel.y() + y && j < dim_voxel_map.y() && !found; j++)
							{
								for (uint32_t k = z; k < dim_super_voxel.z() + z && k < dim_voxel_map.z() && !found; k++)
								{
									...
									if (distance_drawmode == DISTANCE_DRAW_VORONOI_LINEAR)
									{
										...
										unsigned int linear_id = obstacle.x() + dim_voxel_map.x() * (obstacle.y() + dim_voxel_map.y() * (obstacle.z()));
									...
									if (distance_drawmode == DISTANCE_DRAW_VORONOI_SCRAMBLE)
									{
										const Vector3ui obstacle = voxel.getObstacle();
										unsigned int linear_id = obstacle.x() + dim_voxel_map.x() * (obstacle.y() + dim_voxel_map.y() * (obstacle.z()));
										unsigned int scrambled_id = linear_id * 137 + 241; //use swept ids 20-219
			...
			for (uint32_t i = blockIdx.x * blockDim.x + threadIdx.x; i < size; i += blockDim.x * gridDim.x)
			{
				...
				for (size_t t = 0; t < MAX_DRAW_TYPES; ++t)
				{
					...
					if (draw_voxel_type[t] && cube->m_type_vector.getBit(t))
					{
						...
						//            printf("Found voxel at (%f,%f,%f) with voxel type %lu, and sidelength %f\n", x, y, z, t, w);
		...
		void fill_vbo_without_precounting_host(dim3 num_blocks, dim3 thread_per_block, ProbabilisticVoxel* voxelMap, Vector3ui dim_voxel_map, Vector3ui dim_super_voxel, Vector3ui start_voxel, Vector3ui end_voxel, Probability occupancy_threshold, float4* vbo, uint32_t* vbo_offsets, uint32_t* vbo_limits, uint32_t* write_index, uint8_t* draw_voxel_type, uint8_t* prefixes)
		{
			fill_vbo_without_precounting << <num_blocks, thread_per_block >> > (voxelMap, dim_voxel_map, dim_super_voxel, start_voxel, end_voxel, occupancy_threshold, vbo, vbo_offsets, vbo_limits, write_index, draw_voxel_type, prefixes);
		}

		void fill_vbo_without_precounting_host(dim3 num_blocks, dim3 thread_per_block, BitVectorVoxel* voxelMap, Vector3ui dim_voxel_map, Vector3ui dim_super_voxel, Vector3ui start_voxel, Vector3ui end_voxel, uint8_t occupancy_threshold, float4* vbo, uint32_t* vbo_offsets, uint32_t* vbo_limits, uint32_t* write_index, uint8_t* draw_voxel_type, uint8_t* prefixes)
		{
			fill_vbo_without_precounting << <num_blocks, thread_per_block >> > (voxelMap, dim_voxel_map, dim_super_voxel, start_voxel, end_voxel, occupancy_threshold, vbo, vbo_offsets, vbo_limits, write_index, draw_voxel_type, prefixes);
		}

		void fill_vbo_without_precounting_host(dim3 num_blocks, dim3 thread_per_block, DistanceVoxel* voxelMap, Vector3ui dim_voxel_map, Vector3ui dim_super_voxel, Vector3ui start_voxel, Vector3ui end_voxel, visualizer_distance_drawmodes drawmode, float4* vbo, uint32_t* vbo_offsets, uint32_t* vbo_limits, uint32_t* write_index, uint8_t* draw_voxel_type, uint8_t* prefixes)
		{
			fill_vbo_without_precounting << <num_blocks, thread_per_block >> > (voxelMap, dim_voxel_map, dim_super_voxel, start_voxel, end_voxel, drawmode, vbo, vbo_offsets, vbo_limits, write_index, draw_voxel_type, prefixes);
		}

		void fill_vbo_with_cubelist_host(dim3 num_blocks, dim3 thread_per_block, Cube* cubes, uint32_t size, float4* vbo, uint32_t* vbo_offsets, uint32_t* write_index, uint8_t* draw_voxel_type, uint8_t* prefixes)
		{
			fill_vbo_with_cubelist << <num_blocks, thread_per_block >> > (cubes, size, vbo, vbo_offsets, write_index, draw_voxel_type, prefixes);
		}

		void calculate_cubes_per_type_list_host(dim3 num_blocks, dim3 thread_per_block, Cube* cubes, uint32_t size, uint32_t* cubes_per_type, uint8_t* draw_voxel_type, uint8_t* prefixes)
		{
			calculate_cubes_per_type_list << <num_blocks, thread_per_block >> > (cubes, size, cubes_per_type, draw_voxel_type, prefixes);
		}
		...
		{
			find_cubes_by_coordinates << <num_blocks, thread_per_block >> > (cubes, num_cubes, coords, found_cube, found_flag);
		}
