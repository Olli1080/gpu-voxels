...
//#include <gpu_visualization/TemplatedVoxelMapVisual.hpp>
...
void createRightClickMenu()
{
	...
	for (size_t i = 0; i < voxelmapNames.size(); i++)
	{
		...
		glutAddMenuEntry(tmp.str().data(), 300 + static_cast<int>(i));
	}
	...
	for (size_t i = 0; i < voxellistNames.size(); i++)
	{
		...
		glutAddMenuEntry(tmp.str().data(), 400 + static_cast<int>(i));
	}
	...
	for (size_t i = 0; i < octreeNames.size(); i++)
	{
		...
		glutAddMenuEntry(tmp.str().data(), 500 + static_cast<int>(i));
	}
	...
	for (size_t i = 0; i < primArrayNames.size(); i++)
	{
		...
		glutAddMenuEntry(tmp.str().data(), 600 + static_cast<int>(i));
	}
	...
	glutAddMenuEntry("Deactivated", MENU_VISIBILITYTRIGGER_DEACTIVATED);
...
uint32_t getNumberOfVoxelmapsFromSharedMem()
{
	...
	try
	{
		const SharedMemoryManagerVoxelMaps shm_manager_voxelmaps;
...
uint32_t getNumberOfVoxellistsFromSharedMem()
{
	...
	try
	{
		const SharedMemoryManagerVoxelLists shm_manager_voxellists;
...
uint32_t getNumberOfPrimitiveArraysFromSharedMem()
{
	...
	try
	{
		const SharedMemoryManagerPrimitiveArrays shm_manager_prim_arrays;
...
void registerVoxelmapFromSharedMemory(uint32_t index)
{
	try
	{
		...
		if (!error)
		{
			voxelmap::AbstractVoxelMapVisual* voxel_map;
			switch (map_type)
			{
			case MT_BITVECTOR_VOXELMAP:
				voxel_map = new voxelmap::BitVectorVoxelMapVisual(static_cast<BitVectorVoxel*>(dev_data_pointer), dim, voxel_side_length, MT_BITVECTOR_VOXELMAP);
				break;
			case MT_PROBAB_VOXELMAP:
				voxel_map = new voxelmap::ProbVoxelMapVisual(static_cast<ProbabilisticVoxel*>(dev_data_pointer), dim, voxel_side_length, MT_PROBAB_VOXELMAP);
				break;
			case MT_DISTANCE_VOXELMAP:
				voxel_map = new voxelmap::DistanceVoxelMapVisual(static_cast<DistanceVoxel*>(dev_data_pointer), dim, voxel_side_length, MT_DISTANCE_VOXELMAP);
				break;
			default:
				LOGGING_ERROR(
					Visualization,
...
void registerOctreeFromSharedMemory(uint32_t index)
{
	...
	{
		SharedMemoryManagerOctrees shm_manager_octrees;
		const std::string map_name = shm_manager_octrees.getNameOfOctree(index);
...
void registerPrimitiveArrayFromSharedMemory(uint32_t index)
{
	...
	{
		const SharedMemoryManagerPrimitiveArrays shm_manager_prim_arrays;
		const std::string prim_array_name = shm_manager_prim_arrays.getNameOfPrimitiveArray(index);
...
int32_t main(int32_t argc, char* argv[])
{
	...
	const uint32_t num_voxelmaps = getNumberOfVoxelmapsFromSharedMem();
	...
	const uint32_t num_voxellists = getNumberOfVoxellistsFromSharedMem();
	...
	const uint32_t num_octrees = getNumberOfOctreesFromSharedMem();
	...
	const uint32_t num_prim_arrays = getNumberOfPrimitiveArraysFromSharedMem();
	...
	catch (const thrust::system::system_error& e)
	{
		...
		std::filesystem::remove(std::filesystem::path(shmPath + shm_segment_name_octrees));
		std::filesystem::remove(std::filesystem::path(shmPath + shm_segment_name_voxelmaps));
		std::filesystem::remove(std::filesystem::path(shmPath + shm_segment_name_voxellists));
		std::filesystem::remove(std::filesystem::path(shmPath + shm_segment_name_primitive_array));
		std::filesystem::remove(std::filesystem::path(shmPath + shm_segment_name_visualizer));
