...
#include "kernels/VisualizerUtil.h"
/*
...
#include "kernels/VoxelMapVisualizerOperations.h"
...
namespace gpu_voxels {
	namespace visualization {
		...
		Visualizer::Visualizer()
		{
			...
			m_shm_manager_octrees = nullptr;
			m_shm_manager_voxelmaps = nullptr;
			m_shm_manager_voxellists = nullptr;
			m_shm_manager_primitive_arrays = nullptr;
			m_shm_manager_visualizer = nullptr;
		...
		std::vector<std::string> Visualizer::getVoxelMapNames() const
		{
			std::vector<std::string> names;
			names.reserve(m_cur_context->m_voxel_maps.size());
			for (const auto& m_voxel_map : m_cur_context->m_voxel_maps)
				names.push_back(m_voxel_map->map_name());

			return names;
		}

		std::vector<std::string> Visualizer::getVoxelListNames() const
		{
			std::vector<std::string> names;
			names.reserve(m_cur_context->m_voxel_lists.size());
			for (const auto& m_voxel_list : m_cur_context->m_voxel_lists)
				names.push_back(m_voxel_list->map_name());

			return names;
		}

		std::vector<std::string> Visualizer::getOctreeNames() const
		{
			std::vector<std::string> names;
			names.reserve(m_cur_context->m_octrees.size());
			for (const auto& m_octree : m_cur_context->m_octrees)
				names.push_back(m_octree->map_name());

			return names;
		}

		std::vector<std::string> Visualizer::getPrimitiveArrayNames() const
		{
			std::vector<std::string> names;
			names.reserve(m_cur_context->m_prim_arrays.size());
			for (const auto& m_prim_array : m_cur_context->m_prim_arrays)
				names.push_back(m_prim_array->map_name());
		...
		bool Visualizer::initGL(int32_t* argc, char** argv)
		{
			...
			for (auto& m_primitive : m_primitives)
				m_primitive->create(m_cur_context->m_lighting);
		...
		{
			m_interpreter = std::unique_ptr<XMLInterpreter>();

			m_max_mem = m_interpreter->getMaxMem();
			m_max_fps = m_interpreter->getMaxFps();
			m_interpreter->getPrimitives(m_primitives);

			m_cur_context = std::make_unique<VisualizerContext>();
			const bool suc = m_interpreter->getVisualizerContext(m_cur_context);
		...
		bool Visualizer::initializeVisualizer(int& argc, char* argv[])
		{
			...
			constexpr time_t total_wait = 30; // total time used for trying to open shared memory file (seconds)
			constexpr size_t period = 500; // time to sleep between consecutive tries (milliseconds)

			const time_t start = time(nullptr);
			bool success = false;
			while (!success && time(nullptr) < start + total_wait)
			{
				try
				{
					m_shm_manager_visualizer = std::make_unique<SharedMemoryManagerVisualizer>();
					success = true;
				}
			...
			const bool xmlContext = initializeContextFromXML();
			return xmlContext && initGL(&argc, argv);
		}

		void Visualizer::initializeDrawTextFlags()
		{
			if (m_cur_context->m_voxel_maps.empty())
				m_drawVoxelMapText = false;
			if (m_cur_context->m_voxel_lists.empty())
				m_drawVoxelListText = false;
			if (m_cur_context->m_octrees.empty())
				m_drawOctreeText = false;
			if (m_cur_context->m_prim_arrays.empty())
		...
		void Visualizer::generateGLBufferForDataContext(DataContext& con)
		{
			...
			constexpr uint32_t num_voxels = 10;
			const uint32_t default_size = static_cast<uint32_t>(con.m_num_voxels_per_type.size() * num_voxels * SIZE_OF_TRANSLATION_VECTOR);
			//generate the buffer
			GLuint vbo;
			glGenBuffers(1, &vbo);
			glBindBuffer(GL_ARRAY_BUFFER, vbo);
			glBufferData(GL_ARRAY_BUFFER, default_size, nullptr, GL_DYNAMIC_DRAW);
		...
		{
			const size_t req_size = std::max(static_cast<size_t>(100), new_size_byte);
			const size_t cur_size = current_size;
		...
		void Visualizer::resizeGLBufferForWithoutPrecounting(VoxelmapContext& context)
		{
			...
			const size_t new_size_byte = std::accumulate(
				context.m_vbo_segment_voxel_capacities.begin(), 
				context.m_vbo_segment_voxel_capacities.end(), 0) * SIZE_OF_TRANSLATION_VECTOR;
		...
		{
			cudaGraphicsResource* cuda_res = con.m_cuda_ressources;

			m_cur_mem -= con.m_cur_vbo_size;
			const GLuint vbo = con.m_vbo;
			//  std::cout << getDeviceMemoryInfo();
			LOGGING_DEBUG_C(Visualization, Visualizer,
		...
		void Visualizer::createGridVBO() const
		{
			...
			x_max += distance - glm::mod(x_max, distance);
			y_max += distance - glm::mod(y_max, distance);

			std::vector<glm::vec3> data;
			for (float i = 0; i <= x_max; i += distance)
			{/*Insert the lines along the x axis*/
				data.emplace_back(i, 0, 0);
				data.emplace_back(i, y_max, 0);
			}
			for (float i = 0; i <= y_max; i += distance)
			{/*Insert the lines along the y axis*/
				data.emplace_back(0, i, 0);
				data.emplace_back(x_max, i, 0);
			}
			const size_t size = 2.f * ((y_max + x_max) / distance + 2); // two points per line and one line for each x and y value
			assert(data.size() == size);
			const size_t size_in_byte = size * sizeof(glm::vec3);
		...
		void Visualizer::createFocusPointVBO() const
		{
		...
		{
			auto s = std::make_unique<Sphere>(color, pos, radius, 16);
			s->create(m_cur_context->m_lighting);
			m_primitives.push_back(std::unique_ptr<Primitive>(s.release()));
		}

		void Visualizer::createHelperCuboid(glm::vec3 pos, glm::vec3 side_length, glm::vec4 color)
		{
			auto c = std::make_unique<Cuboid>(color, pos, side_length);
			c->create(m_cur_context->m_lighting);
			m_primitives.push_back(std::unique_ptr<Primitive>(c.release()));
		}

		void Visualizer::registerOctree(uint32_t index, std::string map_name)
		{
			if (!m_shm_manager_octrees)
			{
				try
				{
					m_shm_manager_octrees = std::make_unique<SharedMemoryManagerOctrees>();
				}
				catch (boost::interprocess::interprocess_exception&)
				{
					LOGGING_ERROR_C(
			...
			auto con = std::make_unique<CubelistContext>(map_name);
			...
			m_cur_context->m_octrees.emplace_back(std::move(con));
			distributeMaxMemory();

			m_shm_manager_octrees->setOctreeOccupancyThreshold(index, m_cur_context->m_octrees.back()->m_occupancy_threshold);
			uint32_t sdim;
			if (m_shm_manager_octrees->getSuperVoxelSize(sdim))
			{
				m_cur_context->m_dim_svoxel = Vector3ui::Constant(std::max(static_cast<uint32_t>(1), sdim));
				LOGGING_INFO_C(
					Visualization,
					Visualizer,
		...
		void Visualizer::registerVoxelMap(voxelmap::AbstractVoxelMapVisual* map, uint32_t index, std::string map_name)
		{
			if (!m_shm_manager_voxelmaps)
			{
				try
				{
					m_shm_manager_voxelmaps = std::make_unique<SharedMemoryManagerVoxelMaps>();
				}
			...
			auto con = std::make_unique<VoxelmapContext>(map, map_name);
			...
			generateGLBufferForDataContext(*con);
			const Vector3ui d = m_cur_context->m_max_voxelmap_dim = m_cur_context->m_max_voxelmap_dim.cwiseMax(map->getDimensions());

			const glm::vec3 t = m_cur_context->m_camera->getCameraTarget();
			if (t.x == -0.000001f && t.x == t.y && t.y == t.z)
			{
				m_cur_context->m_camera->setCameraTarget(glm::vec3(d.x() / 2.f, d.y() / 2.f, 0.f));
				m_cur_context->m_camera->setCameraTargetOfInitContext(glm::vec3(d.x() / 2.f, d.y() / 2.f, 0.f));
				createFocusPointVBO();
			}
			m_cur_context->m_voxel_maps.emplace_back(std::move(con));
			distributeMaxMemory();
		}

		void Visualizer::registerVoxelList(uint32_t index, std::string map_name)
		{
			if (!m_shm_manager_voxellists)
			{
				try
				{
					m_shm_manager_voxellists = std::make_unique<SharedMemoryManagerVoxelLists>();
				}
				catch (boost::interprocess::interprocess_exception&)
				{
					LOGGING_ERROR_C(
			...
			m_cur_context->m_voxel_lists.emplace_back(std::move(con));
			distributeMaxMemory();
		}

		void Visualizer::registerPrimitiveArray(uint32_t index, std::string prim_array_name)
		{
			if (!m_shm_manager_primitive_arrays)
			{
				try
				{
					m_shm_manager_primitive_arrays = std::make_unique<SharedMemoryManagerPrimitiveArrays>();
				}
				catch (boost::interprocess::interprocess_exception&)
				{
					LOGGING_ERROR_C(
			...
			auto con = std::make_unique<PrimitiveArrayContext>(prim_array_name);
			...
			m_cur_context->m_prim_arrays.emplace_back(std::move(con));
			distributeMaxMemory();
		}
		...
		bool Visualizer::fillGLBufferWithoutPrecounting(VoxelmapContext& context)
		{
			...
			if (context.m_voxelMap->getMapType() == MT_BITVECTOR_VOXELMAP)
			{
				...
				fill_vbo_without_precounting_host(context.num_blocks(), context.threads_per_block(),
			...
			*/
			...
			{
				const uint32_t cur_size = context.m_vbo_segment_voxel_capacities[i];
				const uint32_t req_size = context.m_num_voxels_per_type[i];
				const size_t cur_size_byte = cur_size * SIZE_OF_TRANSLATION_VECTOR;
				const size_t req_size_byte = req_size * SIZE_OF_TRANSLATION_VECTOR;
		...
		{
			calculateNumberOfCubeTypes(*m_cur_context, context);

			context.m_vbo_draw_able = resizeGLBufferForCubelist(context);
			if (!context.m_vbo_draw_able)
				return;

			fillGLBufferWithCubelistWOUpdate(*m_cur_context, context, index);
		...
		void Visualizer::updateStartEndViewVoxelIndices() const
		{
			if (m_cur_context->m_draw_whole_map)
			{
				...
				if (m_cur_context->m_slice_axis == 1) {
					...
					m_cur_context->m_view_end_voxel_pos = Vector3ui(m_cur_context->m_slice_axis_position + 1, static_cast<unsigned>(-1), static_cast<unsigned>(-1));
					return;
				}

				if (m_cur_context->m_slice_axis == 2) {
					m_cur_context->m_view_start_voxel_pos = Vector3ui(0, m_cur_context->m_slice_axis_position, 0);
					m_cur_context->m_view_end_voxel_pos = Vector3ui(static_cast<unsigned>(-1), m_cur_context->m_slice_axis_position + 1, static_cast<unsigned>(-1));
					return;
				}

				if (m_cur_context->m_slice_axis == 3) {
					m_cur_context->m_view_start_voxel_pos = Vector3ui(0, 0, m_cur_context->m_slice_axis_position);
					m_cur_context->m_view_end_voxel_pos = Vector3ui(static_cast<unsigned>(-1), static_cast<unsigned>(-1), m_cur_context->m_slice_axis_position + 1);
					return;
				}
			...
			Vector3ui minVec = m_cur_context->m_max_xyz_to_draw.cwiseMin(convertFromVec3ToVector3ui(glm::max(end_voxel_index, glm::vec3(0.f))));
			m_cur_context->m_view_end_voxel_pos = minVec / m_cur_context->m_dim_svoxel;
			// first divide by dim supervoxel and multiple afterwards to cut off at the correct x,y,z value.
			m_cur_context->m_view_end_voxel_pos = m_cur_context->m_dim_svoxel.asDiagonal() * m_cur_context->m_view_end_voxel_pos;
		...
		bool Visualizer::updateOctreeContext(CubelistContext& context, uint32_t index) const
		{
			Cube* cubes;
			uint32_t size;

			const bool suc = m_shm_manager_octrees->getOctreeVisualizationData(cubes, size, index);
			if (suc)
			{
		...
		bool Visualizer::updateVoxelListContext(CubelistContext& context, uint32_t index) const
		{
			Cube* cubes;
			uint32_t size;

			const bool suc = m_shm_manager_voxellists->getVisualizationData(cubes, size, index);
			if (suc)
			{
				context.setCubesDevicePointer(cubes);
				context.setNumberOfCubes(size);
		 ...
		 *              2 <=> z-axis
		 */
		void Visualizer::renderFrameWithColorMap(uint32_t axis, const DataContext& con) const
		{
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			glUseProgram(m_colormap_programID);

			glm::mat4 VP = m_cur_context->m_camera->getProjectionMatrix() * m_cur_context->m_camera->getViewMatrix();

			const GLuint vpID = glGetUniformLocation(m_colormap_programID, "VP");
			const GLuint axisID = glGetUniformLocation(m_colormap_programID, "axis");
			...
			{
				const uint32_t num_primitive_to_render = con.m_num_voxels_per_type[i];
				if (num_primitive_to_render)
				{
					// uint32_t offset = context->calculateOffset(i);
					const uint32_t offset = con.getOffset(i);
			...
			const DataContext& con) const
		{
			...
			const float v = 256.f * 256.f * pixel[0]/*r*/ + 256.f * pixel[1]/*g*/ + pixel[2]/*b*/;
		 ...
		 *
		 */
		void Visualizer::drawDataContext(const DataContext& context)
		{
			if (!context.m_vbo_draw_able)
				return;
			...
			for (uint32_t i = 0; i < context.m_num_voxels_per_type.size(); ++i)
			{
				const uint32_t num_primitive_to_render = context.m_num_voxels_per_type[i];
				...
				if (num_primitive_to_render && m_cur_context->m_draw_types[context.m_types_segment_mapping[i]])
				{
					...
					{
						if (m_cur_context->m_draw_collision_depth_test_always)
					...
					const uint32_t offset = context.getOffset(i);
					...
					const uint8_t color_index = typeToColorIndex(context.m_types_segment_mapping[i]);
					...
					if (m_cur_context->m_draw_edges_of_triangles)
					{
		...
		void Visualizer::drawGrid() const
		{
			if (m_cur_context->m_grid_vbo != 0)
			{/*Only draw the grid if the VBO was generated*/
				glm::mat4 MVP = m_cur_context->m_camera->getProjectionMatrix() * m_cur_context->m_camera->getViewMatrix()
				...
				const float distance = m_cur_context->m_grid_distance; /*distance between the grid lines*/
				...
				const size_t number_line_points = 2.f * ((y_max + x_max) / distance + 2); // two points per line and one line for each x and z value
		...
		void Visualizer::drawFocusPoint() const
		{
		...
		void Visualizer::drawHelperPrimitives() const
		{
			glm::mat4 MVP = m_cur_context->m_camera->getProjectionMatrix() * m_cur_context->m_camera->getViewMatrix();
			glUniformMatrix4fv(m_vpID, 1, GL_FALSE, glm::value_ptr(MVP));
			glUniform1i(m_interpolationID, GL_FALSE);
			...
			for (const auto& m_primitive : m_primitives)
			{
				...
				glEnable(GL_POLYGON_OFFSET_LINE);
				//set color for the lines.
				glm::vec4 c1 = m_primitive->getColor() * 0.5f;
				glUniform4fv(m_startColorID, 1, glm::value_ptr(c1));

				glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
				m_primitive->draw(1, m_cur_context->m_lighting);
		...
		void Visualizer::drawPrimitivesFromSharedMem() const
		{
			if (!m_shm_manager_primitive_arrays)
				return;
			...
			for (size_t prim_array_num = 0; prim_array_num < m_cur_context->m_prim_arrays.size(); prim_array_num++)
			{
				...
				auto& con = *m_cur_context->m_prim_arrays[prim_array_num];
				...
				if (m_shm_manager_primitive_arrays->hasPrimitiveBufferChanged(prim_array_num))
				{
					...
					if (m_shm_manager_primitive_arrays->getPrimitivePositions(prim_array_num, reinterpret_cast<Vector4f**>(&dev_ptr_positions), con.m_total_num_voxels,
						tmp_prim_type))
					{
						if (con.m_cur_vbo_size != con.m_total_num_voxels * SIZE_OF_TRANSLATION_VECTOR)
						{
							...
							glBindBuffer(GL_ARRAY_BUFFER, con.m_vbo);
							glBufferData(GL_ARRAY_BUFFER, con.m_cur_vbo_size, nullptr, GL_STATIC_DRAW);
							glBindBuffer(GL_ARRAY_BUFFER, 0);
						}
						...
						if (con.m_prim_type != tmp_prim_type)
						{
							...
							if (con.m_prim_type == primitive_array::ePRIM_SPHERE)
							{
								...
								con.m_default_prim = std::unique_ptr<Primitive>(sphere.release());
							...
							else if (con.m_prim_type == primitive_array::ePRIM_CUBOID)
							{
								...
								con.m_default_prim = std::unique_ptr<Primitive>(cuboid.release());
								con.m_default_prim->create(m_cur_context->m_lighting);
							}
							else
							{
				...
				if (con.m_default_prim)
				{
					GLuint color_id;
					glm::mat4 V = m_cur_context->m_camera->getViewMatrix();
					glm::mat4 VP = m_cur_context->m_camera->getProjectionMatrix() * V;
					if (m_cur_context->m_lighting)
					{ // set up the correct variables for the shader with lighting
						glUseProgram(m_lighting_programID);
						glm::mat4 V_inv_trans = glm::transpose(glm::inverse(V));
						glm::vec3 lightpos_world = m_cur_context->m_camera->getCameraPosition()
							+ glm::vec3(1.f) * m_cur_context->m_camera->getCameraRight();
						auto light_intensity = glm::vec3(m_cur_context->m_light_intensity);
					...
					glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), static_cast<void*>(nullptr));
		...
		void Visualizer::drawTextLine(std::string text, int x, int y) const
		{
			...
			for (const char i : text)
				glutBitmapCharacter(GLUT_BITMAP_8_BY_13, (int)i);
		...
		void Visualizer::drawText(std::string text, int x, int y) const
		{
		...
		void Visualizer::renderFunction(void)
		{
			...
			if (m_drawTextAll)
			{
				...
				if (m_drawVoxelMapText)
				{
					...
					for (auto& m_voxel_map : m_cur_context->m_voxel_maps)
					{
						if (!m_voxel_map->m_draw_context)
							displayText << "(" << m_voxel_map->m_map_name << ") ";
						else
							displayText << m_voxel_map->m_map_name << " ";
					}
					displayText << "\n";
				...
				if (m_drawVoxelListText)
				{
					...
					for (auto& m_voxel_list : m_cur_context->m_voxel_lists)
					{
						if (!m_voxel_list->m_draw_context)
							displayText << "(" << m_voxel_list->m_map_name << ") ";
						else
							displayText << m_voxel_list->m_map_name << " ";
					}
					displayText << "\n";
				...
				{
					displayText << "Octrees: ";
					for (auto& m_octree : m_cur_context->m_octrees)
					{
						if (!m_octree->m_draw_context)
							displayText << "(" << m_octree->m_map_name << ") ";
						else
							displayText << m_octree->m_map_name << " ";
					}
					displayText << "\n";
				...
				if (m_drawPrimitiveArrayText)
				{
					...
					for (auto& m_prim_array : m_cur_context->m_prim_arrays)
					{
						if (!m_prim_array->m_draw_context)
							displayText << "(" << m_prim_array->m_map_name << ") ";
						else
							displayText << m_prim_array->m_map_name << " ";
					}
					displayText << "\n";
			...
			{
				glm::vec3 target;
				if (m_shm_manager_visualizer && m_shm_manager_visualizer->getCameraTargetPoint(target)
			...
			if (m_cur_context->m_lighting)
			{
				...
				glUniformMatrix4fv(m_light_vID, 1, GL_FALSE, value_ptr(V));
				glUniformMatrix4fv(m_light_v_inv_transID, 1, GL_FALSE, value_ptr(V_inv_trans));
				glUniform3fv(m_light_interpolationLengthID, 1, glm::value_ptr(glm::vec3(m_cur_context->m_interpolation_length)));
			...
			else
			{
				...
				glUniform3fv(m_interpolationLengthID, 1, glm::value_ptr(glm::vec3(m_cur_context->m_interpolation_length)));
			...
			if (m_use_external_draw_type_triggers && m_shm_manager_visualizer)
			{
				...
				{
					for (auto& m_voxel_map : m_cur_context->m_voxel_maps)
						m_voxel_map->m_has_draw_type_flipped = true;

					for (auto& m_octree : m_cur_context->m_octrees)
						m_octree->m_has_draw_type_flipped = true;

					for (auto& m_voxel_list : m_cur_context->m_voxel_lists)
						m_voxel_list->m_has_draw_type_flipped = true;
			...
			{
				if (m_cur_context->m_voxel_maps[i]->m_voxelMap != nullptr && m_cur_context->m_voxel_maps[i]->m_draw_context)
				{
			...
			if (m_max_fps != 0 && m_delta_time < 1000.f / m_max_fps)
			{
				...
				m_delta_time += wait;
				std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<long long>(wait)));
			}
		}

		void Visualizer::mouseClickFunction(int32_t button, int32_t state, int32_t xpos, int32_t ypos)
		{
			const float speed_multiplier = 80.f;
			...
			{
				const int32_t modi = glutGetModifiers();
		 ...
		 */
		void Visualizer::mouseMotionFunction(int32_t xpos, int32_t ypos) const
		{
		...
		void Visualizer::mousePassiveMotionFunction(int32_t xpos, int32_t ypos) const
		{
			m_cur_context->m_camera->setMousePosition(xpos, ypos);
		}

		void Visualizer::resizeFunction(int32_t width, int32_t height) const
		{
		...
		void Visualizer::rotate_slice_axis() const
		{
			// update axis:
			// 0 none, 1 x, 2 y, 3 z
		...
		void Visualizer::move_slice_axis(int offset) const
		{
			// update position
		...
		void Visualizer::keyboardFunction(unsigned char key, int32_t x, int32_t y)
		{
			...
			const int8_t modi = glutGetModifiers();
			const bool alt_pressed = modi & GLUT_ACTIVE_ALT;
			...
			case 'i':
				...
				m_cur_context->m_draw_collision_depth_test_always = !m_cur_context->m_draw_collision_depth_test_always;
				if (m_cur_context->m_draw_collision_depth_test_always)
			...
			case '-': // decrease light intensity
				m_cur_context->m_light_intensity -= 20.f * multiplier;
				m_cur_context->m_light_intensity = std::max(m_cur_context->m_light_intensity, 0.f);
				break;
			case '0':
			case '1':
			case '2':
			...
			case '9':
				...
				flipDrawType(static_cast<BitVoxelMeaning>(decimal_prefix + decimal_key));
				break;
			}
		}

		void Visualizer::keyboardSpecialFunction(int32_t key, int32_t x, int32_t y) const
		{
			//int8_t map_offset = 0;
		...
		void Visualizer::keyboardFlipVisibility(int8_t index) const
		{
		...
		void Visualizer::keyboardDrawTriangles()
		{
			...
			{
			case 0:
				m_cur_context->m_draw_edges_of_triangles = false;
				m_cur_context->m_draw_filled_triangles = true;
				break;
			case 1:
				m_cur_context->m_draw_edges_of_triangles = true;
		...
		void Visualizer::menuFunction(int value)
		{
			...
			case MENU_DEPTHTEST_ALWAYS:
				m_cur_context->m_draw_collision_depth_test_always = true;
				break;
			case MENU_DEPTHTEST_LEQUAL:
				m_cur_context->m_draw_collision_depth_test_always = false;
				break;
			case MENU_LIGHT_ON:
		...
		void Visualizer::idleFunction() const
		{
			glutPostRedisplay();
		}
		...
		void Visualizer::increaseSuperVoxelSize() const
		{
			Vector3ui d = m_cur_context->m_dim_svoxel;

			d = d + d;
			d = d.cwiseMin(Vector3ui::Constant(64));
			...
			for (const auto& m_voxel_map : m_cur_context->m_voxel_maps)
				m_voxel_map->updateCudaLaunchVariables(d);

			/*to force an update of VBO*/
			m_cur_context->m_camera->setViewChanged(true);
		}

		void Visualizer::decreaseSuperVoxelSize() const
		{
			Vector3ui d = m_cur_context->m_dim_svoxel;
		...
		void Visualizer::flipDrawVoxelmap(uint32_t index) const
		{
			...
			auto& con = *m_cur_context->m_voxel_maps[index];
		...
		void Visualizer::flipDrawOctree(uint32_t index) const
		{
			...
			auto& con = *m_cur_context->m_octrees[index];
			con.m_draw_context = !con.m_draw_context;
			if (con.m_draw_context)
			{
				LOGGING_INFO_C(Visualization, Visualizer,
		...
		void Visualizer::flipDrawVoxellist(uint32_t index) const
		{
			...
			auto& con = *m_cur_context->m_voxel_lists[index];
			con.m_draw_context = !con.m_draw_context;
			if (con.m_draw_context)
			{
				LOGGING_INFO_C(Visualization, Visualizer,
		...
		void Visualizer::flipDrawPrimitiveArray(uint32_t index) const
		{
			...
			auto& con = *m_cur_context->m_prim_arrays[index];
			con.m_draw_context = !con.m_draw_context;
			if (con.m_draw_context)
			{
				LOGGING_INFO_C(Visualization, Visualizer,
		...
		void Visualizer::flipDrawType(BitVoxelMeaning type) const
		{
		...
		{
			const auto start = static_cast<uint8_t>(eBVM_SWEPT_VOLUME_START);
			const auto end = static_cast<uint8_t>(eBVM_SWEPT_VOLUME_END);
			if (m_draw_swept_volumes)
			{
		...
		void Visualizer::copyDrawTypeToDevice() const
		{
		...
		void Visualizer::updateTypesSegmentMapping(DataContext& context) const
		{
		 ...
		 */
		std::string Visualizer::printPositionOfVoxelUnderMouseCursor(int32_t xpos, int32_t ypos) const
		{
			...
			for (uint32_t i = 0; i < m_cur_context->m_voxel_maps.size(); i++)
			{
				auto& con = *m_cur_context->m_voxel_maps[i];
				if (!con.m_draw_context) /*If this voxel map isn't drawn => don't look there for voxels*/
					continue;
			...
			for (uint32_t i = 0; i < m_cur_context->m_octrees.size(); i++)
			{
				auto& con = *m_cur_context->m_octrees[i];
				if (!con.m_draw_context)
			...
			for (uint32_t i = 0; i < m_cur_context->m_voxel_lists.size(); i++)
			{
				auto& con = *m_cur_context->m_voxel_lists[i];
				if (!con.m_draw_context)
			...
			if (found_in_voxelmap)
			{
				...
				if (d.x() > 1 || d.y() > 1 || d.z() > 1)
				{
					...
					n_pos = d.asDiagonal() * n_pos;
					Vector3ui end_pos = n_pos + d - Vector3ui::Constant(1);
				...
				auto& vm_context = *m_cur_context->m_voxel_maps[data_index];
			...
			if (found_in_voxellist)
			{
				...
				auto& vm_context = *m_cur_context->m_voxel_lists[data_index];
		...
		void Visualizer::toggleLighting() const
		{
		 ...
		 */
		void Visualizer::distributeMaxMemory() const
		{
			// as a quick hack, the maps will get 4 times the memory than the primitive arrays
			const uint32_t num_data_structures = (4 * m_cur_context->m_voxel_maps.size()) +
			...
			const size_t mem_per_data_structure = m_max_mem / num_data_structures;

			for (const auto& m_voxel_map : m_cur_context->m_voxel_maps)
				m_voxel_map->m_max_vbo_size = 4 * mem_per_data_structure;

			for (const auto& m_octree : m_cur_context->m_octrees)
				m_octree->m_max_vbo_size = 4 * mem_per_data_structure;

			for (const auto& m_prim_array : m_cur_context->m_prim_arrays)
				m_prim_array->m_max_vbo_size = 1 * mem_per_data_structure;

		}

		std::string Visualizer::printNumberOfVoxelsDrawn() const
		{
			uint32_t num_voxels = 0;

			for (const auto& m_voxel_map : m_cur_context->m_voxel_maps)
				num_voxels += m_voxel_map->m_total_num_voxels;

			for (const auto& m_octree : m_cur_context->m_octrees)
				num_voxels += m_octree->m_total_num_voxels;
		...
		std::string Visualizer::printTotalVBOsizes() const
		{
			size_t total_size = 0;
			for (const auto& m_voxel_map : m_cur_context->m_voxel_maps)
				total_size += m_voxel_map->m_cur_vbo_size;

			for (const auto& m_octree : m_cur_context->m_octrees)
				total_size += m_octree->m_cur_vbo_size;

			for (const auto& m_voxel_list : m_cur_context->m_voxel_lists)
				total_size += m_voxel_list->m_cur_vbo_size;
		...
		std::string Visualizer::printViewInfo() const
		{
			//std::cout << "--------printing view info--------" << std::endl;
			std::stringstream resultString;
			const glm::vec3 vd = m_cur_context->m_dim_view;
			resultString << "dimension of view x: " << vd.x << " y: " << vd.y << " z: " << vd.z << std::endl;

			const Vector3ui vs = m_cur_context->m_view_start_voxel_pos;
			resultString << "m_view_start_voxel_index x: " << vs.x() << " y: " << vs.y() << " z: " << vs.z() << std::endl;
			const Vector3ui ve = m_cur_context->m_view_end_voxel_pos;
			resultString << "m_view_end_voxel_index x: " << ve.x() << " y: " << ve.y() << " z: " << ve.z() << std::endl;

			const glm::vec3 d = m_cur_context->m_camera->getCameraDirection();
